...
def test_quest_runtime_audit_reconciles_stale_active_run_without_worker(temp_home: Path) -> None:
    ...
    assert any(
        ...
        and item.get("abandoned_run_id") == "run-stale-audit-001"
        and item.get("diagnosis_code") == "daemon_no_live_worker"
        for item in events
    )
...
def test_terminal_shutdown_after_stop_is_idempotent(temp_home: Path) -> None:
    ensure_home_layout(temp_home)
    ConfigManager(temp_home).ensure_files()
    app = DaemonApp(temp_home)
    quest = app.quest_service.create("terminal shutdown quest")
    quest_id = quest["quest_id"]
    quest_root = Path(quest["quest_root"])
    ...
    while time.time() < deadline:
        ...
        raise AssertionError("terminal prompt did not become ready before shutdown test")
...
def test_terminal_attach_websocket_smoke_supports_live_python_io(temp_home: Path) -> None:
    ...
    try:
        ...
        with websocket_connect(ws_url, open_timeout=5, close_timeout=2, max_size=None) as websocket:
            ...
            _read_until_contains("File: ds_smoke.txt", timeout=8)
...
def test_quest_control_stop_surfaces_controller_owned_auto_reroute_notice(temp_home: Path) -> None:
    ensure_home_layout(temp_home)
    ConfigManager(temp_home).ensure_files()
    app = DaemonApp(temp_home)
    quest = app.quest_service.create("controller reroute notice quest")
    quest_id = quest["quest_id"]
    quest_root = Path(quest["quest_root"])
    ...
    payload = app.handlers.chat(quest_id, {"text": "Controller should reroute this task.", "source": "web-react"})
    assert payload["ok"] is True
    assert runner.started.wait(timeout=2)
    app.quest_service.update_runtime_state(
        quest_root=quest_root,
    ...
    assert stop_payload["ok"] is True
    assert "MedAutoScience 已接管当前运行" in str(stop_payload["notice"]["message"])
    assert "系统会沿用当前研究上下文继续推进" in str(stop_payload["notice"]["message"])
...
def test_submit_user_message_recovers_stalled_live_turn_and_starts_new_run(
    ...
    assert any(
        ...
        and item.get("diagnosis_code") == "daemon_stalled_live_turn"
        for item in events
    )
...
def test_publication_gate_authorized_controller_work_unit_redrive_starts_runner(temp_home: Path) -> None:
    ...
    ConfigManager(temp_home).ensure_files()
    app = DaemonApp(temp_home)
    quest = app.quest_service.create("publication gate authorized controller redrive quest")
    quest_id = quest["quest_id"]
    quest_root = Path(quest["quest_root"])
    study_root = temp_home / "studies" / "001-risk"
    ...
    class RecordingRunner:
        ...
        def run(self, request):  # noqa: ANN001
            ...
            return RunResult(
                ...
                run_id=request.run_id,
                model=request.model,
                output_text="authorized controller work unit accepted",
                exit_code=0,
                history_root=history_root,
                run_root=run_root,
                stderr_text="",
...
def test_publication_gate_anti_spin_redrive_with_new_controller_intent_starts_runner(
    temp_home: Path,
) -> None:
    ensure_home_layout(temp_home)
    ConfigManager(temp_home).ensure_files()
    app = DaemonApp(temp_home)
    quest = app.quest_service.create("publication gate anti spin redrive controller intent")
    quest_id = quest["quest_id"]
    quest_root = Path(quest["quest_root"])
    study_root = temp_home / "studies" / "002-risk"
    ...
    _write_managed_publication_eval_latest(
        ...
        payload={
            "verdict": {
                "overall_verdict": "blocked",
                "primary_claim_status": "partial",
                "summary": "publication gate still needs concrete MAS redrive targets",
                "stop_loss_pressure": "watch",
            },
            "gaps": [{"gap_id": "gap-001", "gap_type": "claim", "severity": "must_fix"}],
            "recommended_actions": [
                {
                    ...
                    "action_type": "return_to_controller",
                    "priority": "now",
                    "reason": "publication gate emitted a concrete redrive target",
                    "requires_controller_decision": True,
                }
            ],
        },
    ...
    stale_run_id = "run-stale-redrive-001"
    runtime_state_path = quest_root / ".ds" / "runtime_state.json"
    runtime_state = read_json(runtime_state_path, {})
    runtime_state.update(
        {
            ...
            "last_controller_decision_authorization": {
                ...
                "work_unit_id": "analysis_claim_evidence_repair",
                "work_unit_fingerprint": "publication-blockers::redrive-002",
                "dedupe_key": "mas-redrive:002:claim-001",
                "control_intent": "redrive_after_anti_spin",
                "specificity_targets": [
                    {
                        "claim_id": "claim-001",
                        "source_path": str(study_root / "paper" / "claim_evidence_map.json"),
                ...
                "controller_work_unit_lifecycle": {
                    ...
                    "latest_event_type": "same_fingerprint_no_artifact_delta",
    ...
    class RecordingRunner:
        ...
        def run(self, request):  # noqa: ANN001
            ...
            return RunResult(
                ...
                run_id=request.run_id,
                model=request.model,
                output_text="anti-spin redrive accepted",
                exit_code=0,
                history_root=history_root,
                run_root=run_root,
                stderr_text="",
    ...
    events = read_jsonl(quest_root / ".ds" / "events.jsonl")
    skipped = [item for item in events if item.get("type") == "runner.turn_skipped"]
    assert not skipped or skipped[-1].get("terminal_reason") != "same_fingerprint_no_artifact_delta"
...
def test_publication_gate_incomplete_controller_work_unit_authorization_skips_runner(
    temp_home: Path,
    ...
    ConfigManager(temp_home).ensure_files()
    app = DaemonApp(temp_home)
    quest = app.quest_service.create(f"publication gate incomplete controller auth {expected_reason}")
    quest_id = quest["quest_id"]
    quest_root = Path(quest["quest_root"])
    study_root = temp_home / "studies" / "001-risk"
    ...
    class FailingRunner:
        ...
        def run(self, request):  # noqa: ANN001
            raise AssertionError("incomplete controller authorization should not invoke the runner")
...
def test_daemon_retry_exhausts_after_five_attempts(temp_home: Path) -> None:
    ...
    retry_exhausted_events = [item for item in events if item.get("type") == "runner.turn_retry_exhausted"]
    ...
    assert snapshot["retry_state"] is None
    assert snapshot["continuation_policy"] == "wait_for_user_or_resume"
    assert snapshot["continuation_reason"] == "runner_retry_budget_exhausted"
    assert turn_errors
    assert turn_errors[-1].get("diagnosis_code") == "runner_retry_budget_exhausted"
    assert retry_exhausted_events
    assert retry_exhausted_events[-1].get("diagnosis_code") == "runner_retry_budget_exhausted"
    assert not any(
        str(item.get("role") or "") == "assistant"
...
def test_daemon_stops_codex_quota_error_without_retrying_provider_account_blocker(
    temp_home: Path,
) -> None:
    ensure_home_layout(temp_home)
    ConfigManager(temp_home).ensure_files()
    ...
    quest = app.quest_service.create("codex quota error quest")
    quest_id = quest["quest_id"]

    class CodexQuotaFailRunner:
        binary = ""
        ...
        def run(self, request):
            ...
            return RunResult(
                ...
                run_id=request.run_id,
                model=request.model,
                output_text="unexpected status 402 Payment Required: quota exceeded for this account",
                exit_code=1,
                history_root=history_root,
                run_root=run_root,
                stderr_text="",
    ...
    while time.time() < deadline:
        ...
        raise AssertionError("Codex quota failure did not settle into an immediate external account blocker")
    ...
    assert snapshot["retry_state"] is None
    assert snapshot["continuation_policy"] == "wait_for_user_or_resume"
    assert snapshot["continuation_reason"] == "external_codex_upstream_quota_error"
    assert snapshot["status"] == "error"
    assert snapshot["display_status"] == "error"
    assert not any(item.get("type") == "runner.turn_retry_scheduled" for item in events)
    assert turn_errors
    assert turn_errors[-1].get("diagnosis_code") == "codex_upstream_quota_error"
