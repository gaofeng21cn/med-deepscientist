...
def _materialize_submission_minimal_projection(
    ...
    *,
    ...
    (submission_root / "manuscript.docx").write_text("docx", encoding="utf-8")
    (submission_root / "paper.pdf").write_text("%PDF-1.4\n", encoding="utf-8")
    package_files = [
...
def test_submit_paper_bundle_blocks_unmapped_completed_analysis(temp_home: Path) -> None:
    ...
    analysis_result_root = paper_workspace / "experiments" / "analysis-results" / "analysis-gate"
    analysis_result_root.mkdir(parents=True, exist_ok=True)
    (analysis_result_root / "ablation.md").write_text(
        "# Gate ablation\n\nCompleted but not mapped into the paper contract.\n",
        encoding="utf-8",
    )
...
def test_paper_bundle_gate_ignores_manifest_only_analysis_absent_from_current_health_inventory(
    temp_home: Path,
) -> None:
    ensure_home_layout(temp_home)
    ConfigManager(temp_home).ensure_files()
    quest_service = QuestService(temp_home, skill_installer=SkillInstaller(repo_root(), temp_home))
    quest = quest_service.create("paper bundle stale manifest gate quest")
    quest_root = Path(quest["quest_root"])
    artifact = ArtifactService(temp_home)
    ...
    artifact.submit_paper_outline(
        quest_root,
        mode="candidate",
        title="Stale Manifest Gate Outline",
        detailed_outline={
            "title": "Stale Manifest Gate Outline",
            "research_questions": ["RQ-stale-manifest"],
            "experimental_designs": ["Exp-stale-manifest"],
            "sections": [
                {
                    "section_id": "results-stale-manifest",
                    "title": "Stale Manifest Results",
                    "paper_role": "main_text",
                    "claims": ["C1"],
                    "required_items": [],
                    "optional_items": [],
    ...
    artifact.submit_paper_outline(
        ...
        mode="select",
        outline_id="outline-001",
        selected_reason="Use the stale-manifest outline for bundle gate validation.",
    )
    paper_workspace = quest_service.active_workspace_root(quest_root)

    write_json(
        quest_root / ".ds" / "analysis_campaigns" / "analysis-stale-manifest.json",
        {
            "campaign_id": "analysis-stale-manifest",
            "selected_outline_ref": "outline-001",
            "paper_line_id": "paper-line-legacy-comparator",
            "slices": [
                {
                    "slice_id": "legacy-comparator-audit",
                    "title": "Legacy comparator audit",
                    "status": "completed",
                }
            ],
        },
    ...
    health = artifact._paper_contract_health_payload(quest_root, workspace_root=paper_workspace)
    assert health["unmapped_completed_count"] == 0
    assert health["unmapped_completed_items"] == []

    gate_status = artifact._paper_bundle_gate_status(quest_root, workspace_root=paper_workspace)
    assert gate_status["unmapped_completed_items"] == []
    assert gate_status["ok"] is True
...
def test_paper_decision_sync_projects_stop_hold_display_without_changing_canonical_stop(
    temp_home: Path,
    monkeypatch,
) -> None:
    ensure_home_layout(temp_home)
    ConfigManager(temp_home).ensure_files()
    quest_service = QuestService(temp_home, skill_installer=SkillInstaller(repo_root(), temp_home))
    quest = quest_service.create("paper stop hold sync quest")
    quest_root = Path(quest["quest_root"])
    artifact = ArtifactService(temp_home)
    ...
    candidate = artifact.submit_paper_outline(
        quest_root,
        mode="candidate",
        title="Stop Hold Route Outline",
        detailed_outline={
            "title": "Stop Hold Route Outline",
            "research_questions": ["RQ-stop-hold-route"],
            "experimental_designs": ["EXP-stop-hold-route"],
        },
    )
    artifact.submit_paper_outline(
        quest_root,
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: {
            ...
            "completion_approval_ready": False,
            "completion_blocking_reasons": [],
            "blocking_reasons": ["publication gate keeps this line on hold"],
    ...
    recorded = artifact.record(
        quest_root,
        {
            ...
            "action": "stop",
            "next_stage": "decision",
            "protocol_step": "stop_hold_after_aligned_gate_refresh",
            "reason": "Current publication gate still blocks continuation, so the line stays on stop-hold.",
            "summary": "Route remains on stop-hold.",
            "details": {"operational_route": "decision/stop-hold"},
        },
        workspace_root=paper_workspace,
    )
    ...
    assert recorded["ok"] is True
    assert payload["continuation_display_action"] == "stop-hold"
    assert payload["continuation_canonical_action"] == "stop"
    assert "Current Quest Route: `decision` / `stop-hold`" in refreshed_status
    assert "- Quest-Level Next Action: `stop-hold`" in refreshed_status
    assert "- Canonical Recorded Action: `stop`" in refreshed_status
    assert "- Quest-level next step: `decision` / `stop-hold`" in refreshed_summary
...
def test_progress_interact_does_not_refresh_paper_line_sync_after_route_decision(
    ...
    ConfigManager(temp_home).ensure_files()
    quest_service = QuestService(temp_home, skill_installer=SkillInstaller(repo_root(), temp_home))
    quest = quest_service.create("paper progress sync guard quest")
    quest_root = Path(quest["quest_root"])
    artifact = ArtifactService(temp_home)
    ...
    candidate = artifact.submit_paper_outline(
        quest_root,
        mode="candidate",
        title="Progress Sync Guard Outline",
        detailed_outline={
            "title": "Progress Sync Guard Outline",
            "research_questions": ["RQ-progress-sync-guard"],
            "experimental_designs": ["EXP-progress-sync-guard"],
        },
    )
    artifact.submit_paper_outline(
        quest_root,
    ...
    artifact.record(
        quest_root,
        {
            ...
            "action": "stop",
            "next_stage": "decision",
            "reason": "Freeze the paper line and keep the engineering route on the upstream repair handoff.",
            "summary": "Quest route is already decided; ordinary progress updates should not rewrite paper-line sync.",
        },
        workspace_root=paper_workspace,
    )

    status_before = (quest_root / "status.md").read_text(encoding="utf-8")
    summary_before = (quest_root / "SUMMARY.md").read_text(encoding="utf-8")
    paper_state_before = read_json(quest_root / "paper" / "paper_line_state.json")
    monkeypatch.setattr("deepscientist.artifact.service.utc_now", lambda: "2099-01-01T00:00:00+00:00")

    progress = artifact.interact(
        quest_root,
        kind="progress",
        message="老师，我正在继续上游修复定位，不需要改写论文线状态摘要。",
        deliver_to_bound_conversations=False,
        include_recent_inbound_messages=False,
    )

    assert progress["status"] == "ok"
    assert (quest_root / "status.md").read_text(encoding="utf-8") == status_before
    assert (quest_root / "SUMMARY.md").read_text(encoding="utf-8") == summary_before
    assert read_json(quest_root / "paper" / "paper_line_state.json") == paper_state_before
...
def test_render_paper_line_status_and_summary_surface_metadata_only_blocker_details(
    temp_home: Path,
) -> None:
    ensure_home_layout(temp_home)
    ConfigManager(temp_home).ensure_files()
    ...
    payload = {
        ...
        "recommended_action": "finish_proofing_and_submission_checks",
        "paper_branch": "paper/main",
        "updated_at": "2026-04-18T13:31:37+00:00",
        "paper_line_id": "paper-main-outline-001-run",
        "title": "Route Sync Outline",
        "selected_outline_ref": "outline-001",
        "contract_ok": True,
        ...
        "submission_blocking_item_details": [
            {
                ...
                "status": "open_blocker",
                "next_action": (
                    "Fill the metadata confirmation template, copy the confirmed wording into the title-page "
                    "and manuscript-facing declaration surfaces, then run one targeted package re-audit plus "
                    "one changed-page visual recheck."
                ),
            }
        ],
        "metadata_closeout": {
    ...
    assert "- Open Submission Blocker: `CHK-011`" in status_text
    assert "- CHK-011 Meaning: Title page, manuscript Declarations, authorship metadata, affiliations, and final submission declarations still require the externally confirmed packet" in status_text
    assert "- Field Split: `15` open fields (`11` externally required, `2` local-candidate fields still need explicit external confirmation, `2` optional)" in status_text
    assert "- Next Local Action After Packet Arrival: Fill the metadata confirmation template, copy the confirmed wording into the title-page and manuscript-facing declaration surfaces, then run one targeted package re-audit plus one changed-page visual recheck." in status_text
    assert "submission packaging checklist still has 1 blocking item(s)" not in status_text
    assert "- Open Submission Blocker: `CHK-011`" in summary_text
    assert "- Field Split: `15` open fields (`11` externally required, `2` local-candidate fields still need explicit external confirmation, `2` optional)" in summary_text
    assert "submission packaging checklist still has 1 blocking item(s)" not in summary_text
...
def test_get_paper_contract_health_accepts_schema_v1_submission_checklist_surface(temp_home: Path) -> None:
    ...
    assert health["submission_blocking_item_details"] == [
        {
            "item_id": "CHK-011",
            "title": "Title-page packet needs external metadata",
            "status": "blocked_external_metadata_only",
    ...
    assert health["metadata_closeout"] == {
        "status": "external_metadata_blocker_only",
        "field_status_summary": {
            "total_open_fields": 15,
            "external_confirmation_required": 11,
...
def test_get_paper_contract_health_keeps_nonfinal_write_review_maintenance_route_when_gate_is_clear(
    temp_home: Path,
) -> None:
    ensure_home_layout(temp_home)
    ConfigManager(temp_home).ensure_files()
    quest_service = QuestService(temp_home, skill_installer=SkillInstaller(repo_root(), temp_home))
    quest = quest_service.create("artifact nonfinal write review maintenance quest")
    quest_root = Path(quest["quest_root"])
    artifact = ArtifactService(temp_home)
    ...
    write_json(
        ...
        {
            "outline_id": "outline-001",
            "title": "Artifact Nonfinal Maintenance Outline",
            "sections": [],
        },
    )
    write_json(
        paper_root / "paper_line_state.json",
        {
            "paper_line_id": "paper-line-nonfinal-maintenance",
            "paper_branch": "paper/nonfinal-maintenance",
            "selected_outline_ref": "outline-001",
            "title": "Artifact Nonfinal Maintenance Outline",
            "draft_status": "present",
            "bundle_status": "present",
    ...
    write_json(
        review_root / "submission_checklist.json",
        {
            ...
            "package_status": "review_bundle_synced_nonfinal_write_review",
            "handoff_ready": False,
            "blocking_items": [],
        },
    )
    ...
    write_json(
        publication_eval_root / "latest.json",
        {
            ...
            "recommended_actions": [
                {
                    "action_type": "continue_bundle_stage",
                }
            ],
        },
    )
    ...
    assert health["submission_checklist_handoff_ready"] is False
    assert health["submission_minimal_ready"] is True
    assert health["audit_package_ready"] is True
    assert health["finalize_ready"] is False
    assert health["recommended_next_stage"] == "write"
    assert health["recommended_action"] == "continue_nonfinal_write_review_maintenance"
    assert health["human_milestone"]["milestone_id"] == "content_complete_nonfinal_write_review_maintenance"
    assert health["human_milestone"]["objective_metadata_only_remaining"] is False
    assert health["human_milestone"]["remaining_scope"] == "nonfinal_write_review_support"
    assert "只差客观信息" not in health["human_milestone"]["status_summary_zh"]
    assert "only objective submission metadata remains" not in health["human_milestone"]["status_summary_en"]
    assert "non-final write/review maintenance" in health["human_milestone"]["status_summary_en"]
    assert (
        health["status_narration_contract"]["remaining_scope"]["scope_id"]
        == "nonfinal_write_review_support"
    )
    assert (
        health["status_narration_contract"]["remaining_scope"]["objective_metadata_only_remaining"]
        is False
    )
    joined = " ".join(health["blocking_reasons"])
    assert "non-final for write/review maintenance" in joined
    ...
    assert state["recommended_action"] == "continue_nonfinal_write_review_maintenance"
    assert "continue_nonfinal_write_review_maintenance" in status_text
    assert "finish_proofing_and_submission_checks" not in status_text
    assert "continue_nonfinal_write_review_maintenance" in summary_text
    assert "只差客观信息" not in status_text
    assert "只差客观信息" not in summary_text


def test_get_paper_contract_health_reads_nonblocking_metadata_closeout_from_submission_minimal_manifest(
    temp_home: Path,
) -> None:
    ensure_home_layout(temp_home)
    ConfigManager(temp_home).ensure_files()
    quest_service = QuestService(temp_home, skill_installer=SkillInstaller(repo_root(), temp_home))
    quest = quest_service.create("artifact submission minimal metadata closeout quest")
    quest_root = Path(quest["quest_root"])
    artifact = ArtifactService(temp_home)
    ...
    write_json(
        ...
        {
            "outline_id": "outline-001",
            "title": "Artifact Submission Minimal Metadata Closeout Outline",
            "sections": [],
        },
    )
    write_json(
        paper_root / "paper_line_state.json",
        {
            "paper_line_id": "paper-line-submission-minimal-metadata-closeout",
            "paper_branch": "paper/submission-minimal-metadata-closeout",
            "selected_outline_ref": "outline-001",
            "title": "Artifact Submission Minimal Metadata Closeout Outline",
            "draft_status": "present",
            "bundle_status": "present",
            ...
            "non_blocking_followups": [
                {
                    ...
                    "status": "conditional",
                    "notes": "Only needed after a concrete target journal or template is chosen.",
                },
                {
                    "key": "objective_metadata_closeout",
                    "status": "pending_non_blocking",
                    "notes": "Authors and declarations can be completed later without reopening the package.",
                },
            ],
        },
    )
    ...
    submission_manifest["metadata_closeout"] = {
        "status": "non_blocking_followup_only",
        "summary": (
            "The evidence-bearing manuscript package is complete for the current write-stage route; "
            "journal-template page proof remains conditional and objective submission metadata can be "
            "completed later without reopening manuscript science or package materialization."
        ),
        "non_blocking_followups": [
            {
                "key": "journal_template_page_proof",
    ...
    assert state["metadata_closeout"] == health["metadata_closeout"]
    assert "Metadata Closeout:" in status_text
    assert "journal-template page proof remains conditional" in status_text
    assert "Metadata Closeout:" in summary_text
    assert "objective submission metadata can be completed later" in summary_text


def test_get_paper_contract_health_does_not_promote_optimistic_publication_gate_hint_into_local_blocker(
    temp_home: Path,
) -> None:
    ensure_home_layout(temp_home)
    ConfigManager(temp_home).ensure_files()
    quest_service = QuestService(temp_home, skill_installer=SkillInstaller(repo_root(), temp_home))
    quest = quest_service.create("artifact optimistic publication gate hint quest")
    quest_root = Path(quest["quest_root"])
    artifact = ArtifactService(temp_home)
    ...
    write_json(
        ...
        {
            "outline_id": "outline-001",
            "title": "Artifact Optimistic Publication Gate Hint Outline",
            "sections": [],
        },
    )
    write_json(
        paper_root / "paper_line_state.json",
        {
            "paper_line_id": "paper-line-artifact-optimistic-publication-gate-hint",
            "paper_branch": "paper/artifact-optimistic-publication-gate-hint",
            "selected_outline_ref": "outline-001",
            "title": "Artifact Optimistic Publication Gate Hint Outline",
            "draft_status": "present",
            "bundle_status": "present",
    ...
    assert any("The title-page packet still needs externally confirmed final author order." in item for item in health["blocking_reasons"])


def test_get_paper_contract_health_keeps_nonfinal_write_review_route_when_publication_eval_is_promising(
    temp_home: Path,
) -> None:
    ensure_home_layout(temp_home)
    ConfigManager(temp_home).ensure_files()
    quest_service = QuestService(temp_home, skill_installer=SkillInstaller(repo_root(), temp_home))
    quest = quest_service.create("artifact promising publication gate nonfinal maintenance quest")
    quest_root = Path(quest["quest_root"])
    artifact = ArtifactService(temp_home)
    ...
    write_json(
        ...
        {
            "outline_id": "outline-001",
            "title": "Artifact Promising Publication Gate Nonfinal Maintenance Outline",
            "sections": [],
        },
    )
    write_json(
        paper_root / "paper_line_state.json",
        {
            "paper_line_id": "paper-line-artifact-promising-publication-gate-nonfinal-maintenance",
            "paper_branch": "paper/artifact-promising-publication-gate-nonfinal-maintenance",
            "selected_outline_ref": "outline-001",
            "title": "Artifact Promising Publication Gate Nonfinal Maintenance Outline",
            "draft_status": "present",
            "bundle_status": "present",
    ...
    joined = " ".join(health["blocking_reasons"])
    assert "non-final for write/review maintenance" in joined
    assert "managed publication gate blocks completion" not in joined
...
def test_get_paper_contract_health_keeps_optional_gap_nonclear_when_continue_same_line_requires_controller_decision(
    temp_home: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ensure_home_layout(temp_home)
    ConfigManager(temp_home).ensure_files()
    quest_service = QuestService(temp_home, skill_installer=SkillInstaller(repo_root(), temp_home))
    quest = quest_service.create("artifact optional publication eval gap still requires controller decision")
    quest_root = Path(quest["quest_root"])
    artifact = ArtifactService(temp_home)
    ...
    write_json(
        ...
        {
            "outline_id": "outline-001",
            "title": "Artifact Optional Publication Eval Gate Requires Controller Decision Outline",
            "sections": [],
        },
    )
    write_json(
        paper_root / "paper_line_state.json",
        {
            "paper_line_id": "paper-line-artifact-optional-publication-eval-gap-controller-decision",
            "paper_branch": "paper/artifact-optional-publication-eval-gap-controller-decision",
            "selected_outline_ref": "outline-001",
            "title": "Artifact Optional Publication Eval Gate Requires Controller Decision Outline",
            "draft_status": "present",
            "bundle_status": "present",
    ...
    assert health["finalize_ready"] is False
    assert health["recommended_next_stage"] == "write"
    assert health["recommended_action"] == "return_to_publishability_gate"
    assert "controller decision" in " ".join(health["blocking_reasons"])

    state = artifact._write_paper_line_state(quest_root)
    assert state["managed_publication_gate_clear"] is False
    assert state["recommended_next_stage"] == "write"
    assert state["recommended_action"] == "return_to_publishability_gate"

    stale_snapshot = json.loads(json.dumps(artifact.quest_service.snapshot(quest["quest_id"])))
    stale_blocker = "submission-minimal package is incomplete (`paper/submission_minimal/`; missing: submission_manifest.json, paper.pdf)"
    stale_snapshot["paper_contract_health"]["blocking_reasons"] = [stale_blocker]
    stale_snapshot["paper_contract_health"]["recommended_action"] = "finish_proofing_and_submission_checks"
    stale_snapshot["paper_contract_health"]["status_narration_contract"]["current_blockers"] = [stale_blocker]
    stale_snapshot["paper_contract_health"]["status_narration_contract"]["next_step"] = (
        "finish_proofing_and_submission_checks"
    )
    monkeypatch.setattr(artifact.quest_service, "snapshot", lambda _quest_id: stale_snapshot)

    quest_state = artifact.get_quest_state(quest_root, detail="summary")
    global_status = artifact.get_global_status(quest_root, detail="brief", locale="zh")

    assert quest_state["quest_state"]["paper_contract_health"]["blocking_reasons"] == health["blocking_reasons"]
    assert (
        quest_state["quest_state"]["paper_contract_health"]["recommended_action"]
        == "return_to_publishability_gate"
    )
    assert (
        global_status["global_status"]["paper_contract_health"]["recommended_action"]
        == "return_to_publishability_gate"
    )
    assert (
        global_status["global_status"]["status_narration_contract"]["current_blockers"]
        == health["blocking_reasons"]
    )


def test_get_paper_contract_health_prioritizes_controller_gate_hold_over_local_submission_minimal_blocker(
    temp_home: Path,
) -> None:
    ensure_home_layout(temp_home)
    ConfigManager(temp_home).ensure_files()
    quest_service = QuestService(temp_home, skill_installer=SkillInstaller(repo_root(), temp_home))
    quest = quest_service.create("artifact controller gate hold outranks local submission minimal blocker")
    quest_root = Path(quest["quest_root"])
    artifact = ArtifactService(temp_home)
    ...
    write_json(
        ...
        {
            "outline_id": "outline-001",
            "title": "Artifact Controller Gate Hold Outline",
            "sections": [],
        },
    )
    write_json(
        paper_root / "paper_line_state.json",
        {
            "paper_line_id": "paper-line-artifact-controller-gate-hold",
            "paper_branch": "paper/artifact-controller-gate-hold",
            "selected_outline_ref": "outline-001",
            "title": "Artifact Controller Gate Hold Outline",
            "draft_status": "present",
            "bundle_status": "present",
    ...
    write_json(
        ...
        {
            "schema_version": 1,
            "overall_status": "proof_ready",
            "package_status": "proof_ready",
            "handoff_ready": True,
            "blocking_items": [],
        },
    )
    ...
    submission_root = paper_root / "submission_minimal"
    submission_root.mkdir(parents=True, exist_ok=True)
    (submission_root / "manuscript.docx").write_text("placeholder docx", encoding="utf-8")
    ...
    write_json(
        publication_eval_root / "latest.json",
        {
            ...
            "gaps": [
                {
                    "summary": "unmanaged_submission_surface_present",
                    "severity": "must_fix",
                    "gap_type": "delivery",
                },
                {
    ...
    assert health["recommended_next_stage"] == "write"
    assert health["recommended_action"] == "return_to_publishability_gate"
    assert health["blocking_reasons"][0].startswith("managed publication gate blocks completion")
    assert any(
        "submission-minimal package is incomplete" in item for item in health["blocking_reasons"]
    )
    assert (
        health["status_narration_contract"]["current_blockers"][0]
        == health["blocking_reasons"][0]
    )

    state = artifact._write_paper_line_state(quest_root)
    assert state["recommended_action"] == "return_to_publishability_gate"
    assert state["blocking_reasons"][0] == health["blocking_reasons"][0]
    ...
    assert (
        global_status["global_status"]["status_narration_contract"]["current_blockers"][0]
        == health["blocking_reasons"][0]
    )
...
def test_get_paper_contract_health_blocks_finalize_when_submission_minimal_omits_display_exports(
    temp_home: Path,
) -> None:
    ensure_home_layout(temp_home)
    ConfigManager(temp_home).ensure_files()
    quest_service = QuestService(temp_home, skill_installer=SkillInstaller(repo_root(), temp_home))
    quest = quest_service.create("artifact missing submission display exports quest")
    quest_root = Path(quest["quest_root"])
    artifact = ArtifactService(temp_home)
    ...
    write_json(
        ...
        {
            "outline_id": "outline-001",
            "title": "Artifact Missing Submission Display Exports Outline",
            "sections": [],
        },
    )
    write_json(
        paper_root / "paper_line_state.json",
        {
            "paper_line_id": "paper-line-artifact-missing-submission-display-exports",
            "paper_branch": "paper/artifact-missing-submission-display-exports",
            "selected_outline_ref": "outline-001",
            "title": "Artifact Missing Submission Display Exports Outline",
            "draft_status": "present",
            "bundle_status": "present",
...
def test_get_paper_contract_health_accepts_appendix_table_aliases_in_submission_minimal_projection(
    temp_home: Path,
) -> None:
    ensure_home_layout(temp_home)
    ConfigManager(temp_home).ensure_files()
    quest_service = QuestService(temp_home, skill_installer=SkillInstaller(repo_root(), temp_home))
    quest = quest_service.create("artifact submission appendix table naming quest")
    quest_root = Path(quest["quest_root"])
    artifact = ArtifactService(temp_home)
    ...
    write_json(
        ...
        {
            "outline_id": "outline-001",
            "title": "Artifact Submission Appendix Table Naming Outline",
            "sections": [],
        },
    )
    write_json(
        paper_root / "paper_line_state.json",
        {
            "paper_line_id": "paper-line-artifact-submission-appendix-table-naming",
            "paper_branch": "paper/artifact-submission-appendix-table-naming",
            "selected_outline_ref": "outline-001",
            "title": "Artifact Submission Appendix Table Naming Outline",
            "draft_status": "present",
            "bundle_status": "present",
    ...
    (submission_root / "manuscript.docx").write_text("docx", encoding="utf-8")
    (submission_root / "paper.pdf").write_text("%PDF-1.4\n", encoding="utf-8")
    (submission_root / "figures" / "Figure1.png").write_text("figure", encoding="utf-8")
    (submission_root / "tables" / "Table1.csv").write_text("a,b\n1,2\n", encoding="utf-8")
    (submission_root / "tables" / "AppendixTable1.csv").write_text("a,b\n1,2\n", encoding="utf-8")
    write_json(
        submission_root / "submission_manifest.json",
        {
            "manuscript": {
            ...
            "tables": [
                {
                    "table_id": "T1",
                    "paper_role": "main_text",
                    "output_paths": ["paper/submission_minimal/tables/Table1.csv"],
                },
                {
                    "table_id": "TA1",
                    "paper_role": "supplementary",
                    "output_paths": ["paper/submission_minimal/tables/AppendixTable1.csv"],
                },
            ],
        },
    )
    ...
    write_json(
        paper_root / "table_catalog.json",
        {
            "tables": [
                ...
                {
                    "table_id": "TA1",
                    "title": "Appendix public evidence counts",
                },
            ]
        },
    )
    ...
    assert health["submission_minimal_materialized_main_text_figure_count"] == 1
    assert health["submission_minimal_missing_main_text_figure_ids"] == []
    assert health["submission_minimal_expected_table_count"] == 2
    assert health["submission_minimal_materialized_table_count"] == 2
    assert health["submission_minimal_missing_table_ids"] == []
    assert health["submission_minimal_ready"] is True
...
def test_get_paper_contract_health_routes_back_to_write_when_result_display_surface_is_setup_only(
    ...
    ConfigManager(temp_home).ensure_files()
    quest_service = QuestService(temp_home, skill_installer=SkillInstaller(repo_root(), temp_home))
    quest = quest_service.create("artifact display sufficiency quest")
    quest_root = Path(quest["quest_root"])
    artifact = ArtifactService(temp_home)
    paper_root = quest_root / "paper"
    paper_root.mkdir(parents=True, exist_ok=True)
    write_json(
        ...
        {
            "outline_id": "outline-001",
            "title": "Artifact Display Sufficiency Outline",
            "sections": [],
        },
    )
    write_json(
        paper_root / "paper_line_state.json",
        {
            "paper_line_id": "paper-line-artifact-display-sufficiency",
            "paper_branch": "paper/artifact-display-sufficiency",
            "selected_outline_ref": "outline-001",
            "title": "Artifact Display Sufficiency Outline",
            "draft_status": "present",
            "bundle_status": "present",
    ...
    write_json(
        ...
        {
            "status": "resolved",
            "study_root": str(temp_home / "studies" / "001-risk"),
            "publication_profile": "general_medical_journal",
...
def test_get_paper_contract_health_accepts_result_display_when_catalog_materializes_main_text_figure_outside_contract_shell_plan(
    temp_home: Path,
) -> None:
    ensure_home_layout(temp_home)
    ConfigManager(temp_home).ensure_files()
    quest_service = QuestService(temp_home, skill_installer=SkillInstaller(repo_root(), temp_home))
    quest = quest_service.create("artifact display sufficiency fallback quest")
    quest_root = Path(quest["quest_root"])
    artifact = ArtifactService(temp_home)
    paper_root = quest_root / "paper"
    paper_root.mkdir(parents=True, exist_ok=True)
    write_json(
        ...
        {
            "outline_id": "outline-001",
            "title": "Artifact Display Sufficiency Fallback Outline",
            "sections": [],
        },
    )
    write_json(
        paper_root / "paper_line_state.json",
        {
            "paper_line_id": "paper-line-display-sufficiency-fallback",
            "paper_branch": "paper/display-sufficiency-fallback",
            "selected_outline_ref": "outline-001",
            "title": "Artifact Display Sufficiency Fallback Outline",
            "draft_status": "present",
            "bundle_status": "present",
...
def test_get_paper_contract_health_resynchronizes_stale_paper_line_mirrors(
    ...
    ConfigManager(temp_home).ensure_files()
    quest_service = QuestService(temp_home, skill_installer=SkillInstaller(repo_root(), temp_home))
    quest = quest_service.create("paper line mirror resync quest")
    quest_root = Path(quest["quest_root"])
    artifact = ArtifactService(temp_home)
    ...
    write_json(
        paper_root / "paper_line_state.json",
        {
            "paper_line_id": "paper-line-mirror-resync",
            "paper_branch": "paper/mirror-resync",
            "selected_outline_ref": "outline-001",
            "title": "Mirror Resync Outline",
            "contract_ok": False,
            "writing_ready": False,
            "audit_package_ready": True,
            "finalize_ready": False,
            "unmapped_completed_count": 2,
            "recommended_next_stage": "write",
            "recommended_action": "sync_paper_contract",
            "blocking_reasons": [
                "completed analysis remains unmapped into the paper contract",
                "submission packaging checklist is not marked handoff-ready (write_review_maintenance_nonfinal, review_bundle_synced_nonfinal_write_review)",
            ],
            "draft_status": "present",
            "bundle_status": "present",
            "updated_at": "2026-04-03T00:00:00Z",
    ...
    (quest_root / "status.md").write_text(
        "\n".join(
            [
                "# Status",
                "",
                "Paper Line Local Recommendation: `write` / `sync_paper_contract` on `paper/mirror-resync`.",
                "",
                "- Contract: `blocked`",
                "- Writing Readiness: `blocked`",
                "",
                "## Blocking Reasons",
                "",
                "- completed analysis remains unmapped into the paper contract",
    ...
    (quest_root / "SUMMARY.md").write_text(
        "\n".join(
            [
                "# Summary",
                "",
                "Paper Line Summary: Mirror Resync Outline on `paper/mirror-resync`.",
                "",
                "- Writing readiness: `blocked`",
                "- Next step: `write` / `sync_paper_contract`",
                "",
                "## Current Blockers",
                "",
                "- completed analysis remains unmapped into the paper contract",
    ...
    campaign_id = "analysis-mirror-resync"
    campaign_manifest_root = quest_root / ".ds" / "analysis_campaigns"
    campaign_manifest_root.mkdir(parents=True, exist_ok=True)
    write_json(
        campaign_manifest_root / f"{campaign_id}.json",
        {
            ...
            "selected_outline_ref": "outline-001",
            "paper_line_id": "paper-line-mirror-resync",
            "paper_line_branch": "paper/mirror-resync",
            "slices": [
                {
                    "slice_id": "maint-check",
                    "item_id": "EXP-MAINT-001",
    ...
    assert persisted["contract_ok"] == health["contract_ok"]
    assert persisted["writing_ready"] == health["writing_ready"]
    assert persisted["unmapped_completed_count"] == health["unmapped_completed_count"]
    assert persisted["recommended_action"] == health["recommended_action"]
    assert "sync_paper_contract" not in status_text
    assert "sync_paper_contract" not in summary_text
    assert "completed analysis remains unmapped into the paper contract" not in status_text
    assert "completed analysis remains unmapped into the paper contract" not in summary_text
    assert "内容里程碑：已达成" in status_text
    assert "当前可以给人审阅" in summary_text
