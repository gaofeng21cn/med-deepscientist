...
MedDeepScientist 已经暴露出足够多的持久运行时状态，因此你可以在不改 core runtime 的前提下，构建一层外部治理控制器。

这篇文档说明一种适合当前 frozen fork 的最小 public pattern：

- 通过 durable runtime surface 检查 quest 最近状态
- 判断当前 run 是否应该继续
- 通过 quest mailbox 注入一条下一步 routed message
- 必要时通过 `quest_control` 停止当前 run
- 写出一份 durable report，解释为什么 guard 被触发

对于医疗工作流，这层外部治理通常落在 `MedAutoScience` 或其他 controller tier，MedDeepScientist 继续承担稳定 runtime contract。

## 什么时候适合用 external controller

当某条规则满足下面特征时，适合放到外层 controller：

- 明显是项目级或实验室级规则
- 硬编码进全局 prompts 或 skills 的成本偏高
- 更像外层治理逻辑，而不是 core runtime 默认行为

常见例子：

- paper-facing 写作前的 publishability admission 规则
- figure polish 在同一张图上反复 reopen 的空转
- 需要把下一步重新导回有边界路线的 route-drift 策略

## 可以依赖的公共契约

最稳妥的扩展面是现有的 durable runtime contract：

- quest 自有的 runtime truth
  - 优先从 `artifacts/reports/runtime_events/latest.json` 开始读
- 最近 quest 状态
  - quest snapshot、artifact state、connector 可见输出本来就是 durable surface
- quest mailbox
  - 排队中的用户消息位于 `.ds/user_message_queue.json`
- daemon quest control
  - `POST /api/quests/<quest_id>/control`
- controller 自己维护的 durable report
  - 报告写到 quest 树内由 controller 管理的路径
  - `artifacts/reports/external_controller/` 是一条实用约定
...
mailbox 里的消息应该表达结论和下一步路由，不要原样转储日志。
...
-> 写 controller report
-> 必要时 stop 当前 run
-> 往 mailbox 注入一条下一步 routed message
```
...
这类 controller 适合处理一种常见情况：quest 还没有把证据线做扎实，却已经往 paper-facing 写作漂移。

常见输入：

- 最新 verification 结论
- 当前 draft 或 summary 状态
- 最近 baseline 或 utility 结果
...
1. 写出 `artifacts/reports/external_controller/publishability_guard.md`，明确缺什么支撑。
...
这类 controller 适合处理另一种常见情况：同一张图不断 reopen 或 polish，但对主结论已经没有实质推进。
...
- 下一步更有价值的动作已经转向修证据或改正文

常见干预动作：

1. 写出 `artifacts/reports/external_controller/figure_loop_guard.md`，概括当前循环为何低收益。
2. 必要时停止当前 figure 分支。
...
真正随 connector 变化的，主要是给人看的信息颗粒度和呈现方式。
...
如果某个 controller 需要依赖这些东西，说明契约还不够稳定。
