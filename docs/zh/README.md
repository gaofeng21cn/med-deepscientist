...
MedDeepScientist（`med-deepscientist` 仓库）是 `MedAutoScience` 使用的稳定 runtime substrate。

这个仓库起点是一个受控 runtime fork，现在承担更清晰的长线角色：保留已经验证的执行真相、收紧 runtime contract、并通过受控 intake 吸收符合兼容契约的上游增量，从而让 `MedAutoScience` 持续演进且不反复支付兼容漂移成本。
...
- 推进 runtime surface 和 family 合同的持续收敛

在架构上，`MedAutoScience` 仍然是人类与 Agent 的主入口；MedDeepScientist（`med-deepscientist` 仓库）则承担 runtime 工程面、协议收口、以及可审计 capability intake。长线目标是 family 级稳定 runtime surface，以及更清晰的 ownership 边界。
