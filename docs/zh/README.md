...
`docs/zh/` 是 upstream / user-facing guide corpus，服务 fork-local operator workflows。它可以解释继承自 `DeepScientist` 的行为、兼容名称、维护命令和 legacy diagnostic surface，但不定义 MAS owner truth、MAS 默认 runtime dependency、MAS product entry 语义、publication authority、artifact authority 或用户可见研究进度。

需要判断 owner 和稳定边界时，以这些入口为准：

- MAS owner truth 与医学产品入口语义：`MedAutoScience`
- MDS archive/reference 边界：[项目概览](../project.md)、[当前状态](../status.md)、[架构](../architecture.md)
- legacy diagnostic 稳定协议：[稳定 Runtime Protocol](../policies/runtime_protocol.md)
- MAS/MDS 收缩边界：[MAS/MDS 迁移收缩契约](../policies/mas_mds_transition_contract.md)
- 上游 intake 流程：[上游 Intake 指南](../references/upstream_intake.md)
