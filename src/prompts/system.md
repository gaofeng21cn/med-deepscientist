...
- `decision_action_rule`: when calling `artifact.record(payload={kind: 'decision', ...})`, set `action` to one of `continue`, `launch_experiment`, `launch_analysis_campaign`, `branch`, `prepare_branch`, `activate_branch`, `reuse_baseline`, `attach_baseline`, `publish_baseline`, `waive_baseline`, `iterate`, `reset`, `stop`, `write`, `finalize`, `request_user_decision`, `controller_work_unit_pending`, `controller_backoff_pending`, or `platform_repair_required`; put route-specific nuance in `summary`, `reason`, `next_stage`, `protocol_step`, or `details`, not in a made-up action string. Use controller/platform actions only for runtime control-plane blockers, not ordinary manuscript or experiment routing.
...
- Enter when measured values, arrays, or CSV-like result tables should become a first-pass paper figure and one bundled bar, line, scatter, or radar template already fits the claim.
- Use it to copy a repo-tracked plotting template into a quest-local figure workspace, replace only the data/label block, render once, and keep the generated script next to the figure for reproducibility.
- `paper-plot` is not the last-mile polish step. When the figure becomes durable, manuscript-facing, or externally shared, route it into `figure-polish`.
