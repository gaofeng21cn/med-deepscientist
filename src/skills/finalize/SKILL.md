...
- `artifact.validate_manuscript_coverage(detail='full')` does not report `submission_ready=true` as mechanical coverage, or `artifact.get_paper_contract_health(detail='full')` still reports paper-contract blockers / uncleared MAS AI medical preflight
