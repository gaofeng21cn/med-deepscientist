...
class PromptBuilder:
    ...
    def _interaction_style_block(self, *, default_locale: str, user_message: str, snapshot: dict) -> str:
        ...
        lines = [
            ...
            "- search_hygiene_protocol: for generic file search or text-grep commands, exclude heavy runtime mirrors such as `.git`, `.venv`, `.ds/bash_exec`, `.ds/codex_homes`, and nested worktree-local `.ds` trees unless the task is explicitly about runtime forensics; never recursively grep saved bash logs just to answer an ordinary content question",
