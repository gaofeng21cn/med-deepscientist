...
_STALLED_RUNNING_TURN_INTERRUPT_TIMEOUT_SECONDS = 3.0
_CHINESE_DIGIT_MAP = {
    "零": 0,
    "〇": 0,
    "一": 1,
...
class DaemonApp:
        ...
        for item in self.reconciled_quests:
            ...
            try:
                ...
                scheduled = {
                    ...
                    "delayed": bool(resume_payload.get("delayed")),
                    "delay_seconds": resume_payload.get("delay_seconds"),
                }
                event = {
        ...
        *,
        ...
        try:
            ...
            snapshot = self.quest_service.mark_turn_finished(
                ...
                event_kind="runtime_stalled_turn_recovered",
    ...
    def resume_quest(self, quest_id: str, *, source: str = "local") -> dict:
        ...
        elif source.startswith("auto:daemon-recovery") and turn_reason == "auto_continue":
            retry_delay_seconds = self._recovery_retry_delay_seconds(snapshot)
    ...
    def _project_controller_work_unit_block_before_runner(
        ...
        *,
        ...
        if bool(no_progress_classification.get("no_progress_failure")):
            runtime_updates["retry_state"] = snapshot.get("retry_state")
        self.quest_service.update_runtime_state(**runtime_updates)
        append_jsonl(
            quest_root / ".ds" / "events.jsonl",
            {
                "event_id": generate_id("evt"),
